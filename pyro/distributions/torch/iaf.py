...
class InverseAutoregressiveFlow(Transform):
    ...
    def __init__(self, input_dim, hidden_dim, sigmoid_bias=2.0, permutation=None):
        ...
        self._cached_sigma = None, None  # holds a tuple (y, sigma)
    ...
    def _call(self, x):
        ...
        self._cached_sigma = y, sigma
        return y

    def log_abs_det_jacobian(self, x, y):
        y_old, sigma = self._cached_sigma
        if y_old is not y:
            raise KeyError("Transform InverseAutoregressiveFlow expected to find "
